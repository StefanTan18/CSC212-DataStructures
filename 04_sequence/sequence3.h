...
namespace main_savitch_5
{
    ...
    public:
        ...
        value_type current( ) const;
    private:
        node *m_head_ptr;           // List head pointer
        node *m_tail_ptr;           // List tail pointer 
        node *m_cursor;             // Pointer to the node with the current item
        node *m_precursor;          // Pointer to the node before the current item
        size_type m_many_nodes;     // Number of nodes on the list
    };
}
